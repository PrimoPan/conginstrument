...
export const DESTINATION_NOISE_RE =
  /之外|其他时间|这座城|这座城市|城市里|到场|pre|汇报|报告|论文|会议|参加一个|必须|想逛|逛一逛|计划|安排|准备|打算|预算|经费|花费|费用|一天|两天|三天|四天|五天|六天|七天|八天|之前|之后|然后|并且|但是|同时|顺带|顺便|顺路|顺道|其中|其中有|其余|其他时候|海地区|该地区|看球|观赛|比赛|打卡|参观|游览|演讲|发表|所以这|因此|另外|此外|这三天|这几天|人民币/i;

export const PLACE_STOPWORD_RE =
  /我|你|他|她|我们|你们|他们|除了|其他|其中|其中有|其余|时间|必须|到场|之前|之后|然后|安排|计划|准备|打算|预算|经费|花费|费用|参加|会议|汇报|报告|pre|chi|天|行程|顺带|顺便|顺路|顺道|海地区|该地区|看球|观赛|比赛|演讲|发表|所以这|因此|另外|此外|人民币/i;

export const NON_PLACE_TOKEN_RE =
  /^(人民币|预算|经费|花费|费用|准备|打算|计划|安排|行程|旅游|旅行|出行|自由行|参会|开会|会议|汇报|报告|演讲|发表|一天|一周|一月|一人|多人)$/i;
