...
import { looksLikeBroadDestination, looksLikeSoftTripRequest, looksLikeTaskRestart } from "../shared/travelSemantics.js";
...
function shouldCarryPreviousDestinationsForRefinement(params: {
  ...
  const previousIsBroadAnchor =
    ...
    params.previousDestinations.every((dest) => looksLikeBroadDestination(clean(dest, 80)));
  const currentHasBroadAnchor = params.currentDestinations.some((dest) => looksLikeBroadDestination(clean(dest, 80)));
  ...
  if (looksLikeTaskRestart(latestUserText)) return false;
  if (looksLikeSoftTripRequest(latestUserText) && !hierarchicalRefinement) return false;
...
export function buildTaskDetection(params: {
  ...
  const restartText = [latestUserText, clean(params.tripGoalSummary, 220).toLowerCase()]
    .filter(Boolean)
    .join(" ");
  const explicitRestart = looksLikeTaskRestart(restartText);
